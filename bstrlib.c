	...
	if (olen >= b->mlen) {
		...
		if (7 * b->mlen < 8 * b->slen) {
			...
			if (x == NULL) {
				...
				if (NULL == (x = (unsigned char *) bstr__realloc (b->data, len = olen))) {
					return BSTR_ERR;
				}
			}
		} else {
...
unsigned char c0, c1;
	...
	/* Check last position */
	if (i == l && c0 == d1[i] && c1 == d1[i+1]) goto N1;
...
int i;
	if (b1 == NULL || b1->data == NULL || b1->slen <= 0) return BSTR_ERR;
	memset ((void *) cf->content, 0, sizeof (struct charField));
	for (i=0; i < b1->slen; i++) cf->content[(unsigned int) b1->data[i]] = 1;
	return BSTR_OK;
}
