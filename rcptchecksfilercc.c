...
int
lineinfile2 (bstring data, bstring fname)
{
  int retval = 0;
  ...
  for (;;)
    {
      ...
      if (0 == bstrnicmp (data, line,data->slen))
        {
          retval = 1;
          break;
        }
...
main ()
{
  pluginname=bfromcstr ("rcptchecksfile");
  ...
  int smtprcptcount = envtoint ("SMTPRCPTCOUNTALL", 0);
  bstring ctrlfname = envtostr ("CHKUSER_WLFILE");
  bstring addrfname = bfromcstr ("/etc/mail/usuarios2");
  ...
  if (islocal == 1)
    {
      valid = lineinfile2 (smtprcptto, addrfname);
      if (valid == -1)
        {
  ...
  if (valid == 0)
    {
      ...
      _log (bformat ("511 sorry, no mailbox here by that name (%s@%s,%d)",
                     username->data, domain->data, wrongrcptcount));
      exit (0);
    }
